...
本需求源自「LifeOS × LLM-Wiki 融合系統」（一隻阿木木, 2026），該設計提出了完整的 `AI-Log/` 日誌體系。KOS_LLM-Wiki 將其本地化為 `_logs/` 目錄，適配 Codex 作為 AI Agent 的執行環境。
...
1. 讀取 `_logs/sessions/YYYY/` 最近 3 條會話日誌
2. 讀取 `_logs/operations/maintenance.md` 末尾 10 條（系統狀態）
3. 檢查 `_logs/tasks/` 中的未完成任務
4. 將本次會話 ID 記錄到上下文
...
1. 在 `_logs/sessions/YYYY/YYYY-MM-DD.md` 追加新的會話區塊
2. 更新 `_logs/operations/` 中對應的操作日誌
3. 如生成了報告資料，更新 `_logs/reports/`
...
_logs/operations/compile.md     ← 全域編譯操作彙總（引用子庫日誌）
```
...
`AGENTS.md` 中已記錄 `_logs/` 目錄定義。後續如需增加日誌規範，應同步更新 AGENTS.md 中的相關章節。
