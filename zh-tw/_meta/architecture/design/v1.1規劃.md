...
| 審計日誌 (_meta/system/logs/) | ✅ | AI-Log/ 本地化 |
| ADR 架構決策 | ✅ | 新增（超越原設計） |
| C4 模型文件 | ✅ | 新增（超越原設計） |
| 多語言架構 | ✅ | 新增（超越原設計） |
| UDC 分類體系 | ✅ | 新增（超越原設計） |
...
- 輸出：目標目錄 + `_meta/system/logs/operations/triage.md`

**產出物：**
- `AGENTS.md` — 新增 KOS-Triage 分揀規則章節
- `_meta/system/logs/operations/triage.md` — 分揀操作日誌（首次建立）
...
每週回顧自動化：
- 統計本週操作（來自 `_meta/system/logs/operations/`）
- 彙總本週 Wiki 變更
- 產生週報至 `_meta/system/logs/reports/weekly.md`
- 建議下週待辦
...
### 會話結束
1. 寫入會話日誌到 _meta/system/logs/sessions/
2. 更新操作日誌到 _meta/system/logs/operations/
3. 檢查是否有未完成任務需要提醒
```
...
| 會話上下文遺失 | 中 | 低 | _meta/system/logs/sessions/ 提供歷史恢復 |
