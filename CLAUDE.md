...
| `KOS-Query [quick\|standard\|deep] <问题>` | 三级知识库查询（读取范围递进，非检索算法） |
| `KOS-Project` | 创建/管理项目 |
| `KOS-Audit [--list|--findings|--overdue|--type schema]` | 审计管理与执行 |
...
- `_meta/workspace-index.yaml` — 工作区注册表
- `_meta/schema/taxonomy.yaml` — Type & Topic 枚举
- `_meta/schema/frontmatter.yaml` — Frontmatter 字段约束
- `_meta/schema/subsystems.yaml` — 工作区子系统契约
- `_meta/schema/workspace-tools.yaml` — Workspace→Skill 绑定
- `_meta/schema/event-capture.yaml` — Agent 事件采集规格
- `{workspace}/WORKSPACE.md` — 各工作区边界规范
