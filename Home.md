---
created: 2026-06-06
updated: 2026-06-06
udc: 001.8
tags: [home, index, kos]
---

# KOS_LLM-Wiki

**知识组织系统 · Knowledge Organization System**

一个融合 **PARA**、**UDC** 与 **LLM-Wiki** 的现代知识管理中枢。

---

## 快速导航

| 区域                        | 说明                        |
|-----------------------------|-----------------------------|
| [[00-Inbox/\|📥 00-Inbox]]          | 待处理的临时笔记与素材       |
| [[01-Projects/\|📋 01-Projects]]    | 有明确目标的活跃项目         |
| [[02-Areas/\|📂 02-Areas]]          | 持续关注的责任领域           |
| [[03-Resources/\|📚 03-Resources]]  | 主题资料与参考资源           |
| [[04-Archives/\|🗄️ 04-Archives]]   | 已归档的非活跃内容           |
| [[_meta/\|⚙️ _meta]]                | 模板、脚本与附件             |
| [[06-Daily/\|📅 06-Daily]]          | 每日笔记                     |

## 知识体系入口

- [[_索引|📖 总索引]] — 完整的内容索引与导航
- [[03-Resources/PARA/PARA 方法概览|PARA 方法概览]] — 项目-领域-资源-归档
- [[03-Resources/UDC/UDC 概述|UDC 分类体系]] — 国际十进分类法
- [[03-Resources/LLM-Wiki/LLM 基础|LLM 知识库]] — 大语言模型知识

---

> [!info] 使用提示
> 本 Wiki 使用 Obsidian 管理。支持 `[[双向链接]]`、标签系统、属性查询与图谱视图。

## 语言版本

- **English** — [[en/Home|English]]
- **简体中文** — [[Home|简体中文]]
- **繁體中文** — [[zh-tw/首頁|繁體中文]]
