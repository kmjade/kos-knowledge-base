...
| `wiki-fold` | 日志折叠归纳 | 高级 |
| `wiki-cli` | Obsidian CLI 传输包安装 | 基础设施 |
...
### 2. 传输层
`_meta/system/scripts/detect-transport.sh` 自动检测可用通道：Obsidian CLI → MCP → 文件系统。
### 3. 并发控制

`_meta/system/scripts/wiki-lock.sh`：基于 noclobber 原子锁，60s 超时回收。
### 4. 检索系统
...
| _meta/system/templates/ | _templates/ |
...
| 技能格式 | SKILL.md（标准格式） | AGENTS.md + _meta/ai/agents/ |
...
- [[_meta/architecture/design/skills体系分析.md]] — 本 vault skills 现状
