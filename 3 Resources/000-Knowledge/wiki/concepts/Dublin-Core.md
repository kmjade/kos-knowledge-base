---
aliases: [Dublin Core, DC 元數據 Dublin Core Metadata, 都柏林核心]
created: 2026-06-03
type: concept
updated: 2026-06-13
...
> 国际元数据标· ISO 15836 · 15 核心元素 · 用于 Frontmatter 映射

---

## 定义

Dublin Core Metadata Initiative (DCMI) 定义15 元素元数据集，用于描述数字资源的通用元数据标准本系统Frontmatter 标准基于 Dublin Core 映射
...
| 4 | description | 描述 | `aliases` (副标 |
| 5 | publisher | 发布| `source` (URL) |
| 6 | contributor | 贡献| `author` (多人) |
| 7 | date | 日期 | `created` · `updated` |
| 8 | type | 类型 | `type: concept|entity|source` |
| 9 | format | 格式 | `md` (固定) |
| 10 | identifier | 标识| `ddc` + `udc` |
...
- [[KOS]] `part-of` 元數據据是 KOS 的一
- [[Frontmatter]] `implements` Frontmatter 实现 DC
- [[ISO-25964]] `related` 叙词表标
