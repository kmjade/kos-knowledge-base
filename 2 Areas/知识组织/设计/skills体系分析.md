...
### 2. `_meta/ai/agents/` — 自定义角色定义

| 文件 | 内容 | 问题 |
|------|------|------|
| `_meta/ai/agents/life/分析师.md` | 周/月三支柱趋势报告 | ❌ 仅文档，不可执行 |
| `_meta/ai/agents/life/简报员.md` | 晨间简报 + 傍晚回顾 | ❌ 与 AGENTS.md 会话协议重叠 |
| `_meta/ai/agents/life/协作者.md` | 目标进度异常提醒 | ❌ 仅文档，不可执行 |
| `_meta/ai/agents/kos/README.md` | 空占位 | ❌ 未填充 |
...
| 2 | `_meta/ai/agents/life/` 是文档而非可执行 skill | 分析师/简报员/协作者功能无法自动触发 |
| 3 | `_meta/ai/agents/kos/README.md` 为空 | KOS agent 定义缺失 |
| 4 | `_meta/ai/skills/skills.md` 为空 | 技能追踪索引缺失 |
| 5 | `_meta/ai/agents/life/` 与 AGENTS.md 功能重叠 | 会话协议 vs 角色定义，职责不清 |
| 6 | `.opencode/backups/` 冗余备份 | 占用空间，内容重复 |

## 优化方向建议

- **A — 体系整合**：创建 `.claude/skills/`，将 AH Skills 迁移过去，让 Claude Code 可调用
- **B — Life+AI Agent 升级**：将 `_meta/ai/agents/life/` 角色转化为 SKILL.md，解决与 AGENTS.md 的重叠
- **C — KOS Agent 填充**：完善 `_meta/ai/agents/kos/` 的 agent 定义
