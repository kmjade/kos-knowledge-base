...
This specification is derived from the "LifeOS × LLM-Wiki Fusion System" (by 一只阿木木, 2026), which proposed a complete `AI-Log/` architecture. KOS_LLM-Wiki localizes this as the `_meta/system/logs/` directory, adapting it for Codex as the AI Agent runtime environment.
...
1. Read the latest 3 session logs from `_meta/system/logs/sessions/YYYY/`
2. Read the last 10 entries from `_meta/system/logs/operations/maintenance.md`
3. Check `_meta/system/logs/tasks/` for unfinished tasks
...
1. Append new session block to `_meta/system/logs/sessions/YYYY/YYYY-MM-DD.md`
2. Update corresponding operation logs in `_meta/system/logs/operations/`
3. If report data is available, update `_meta/system/logs/reports/`
...
_meta/system/logs/operations/compile.md     ← Global compile summary (references sub-library logs)
```
...
`AGENTS.md` already records the `_meta/system/logs/` directory definition. Subsequent log specification changes should be synced to AGENTS.md.
