...
- **Problem statement**: Dual Inbox directories (`00-Inbox` and `0 Inbox`) caused confusion; leading-zero numbering was inconsistent with the LifeOS fusion system's `0 Inbox` style; `05-Logs/` duplicated the existing `_logs/` directory.
...
- `_logs/` and `_meta/` share consistent naming style
