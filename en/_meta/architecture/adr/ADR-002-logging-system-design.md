...
KOS_LLM-Wiki initially had both `05-Logs/` (from PARA numbering) and `_meta/system/logs/` (from LifeOS convention) directories.
...
- **Options**:
  ...
  2. Unify under `_meta/system/logs/` with `_meta/` as system directory family
  3. Distribute logs to each PARA directory

## Decision

**Chosen: Option 2** — Unify under `_meta/system/logs/` underscore prefix.

**Rationale:**
1. `_meta/system/logs/` matches `_meta/` as system infrastructure, not user content
...
### Mitigation
- `_meta/system/logs/` directory structure:
  ```
  _meta/system/logs/
  ├── operations/
  │   ├── maintenance.md
