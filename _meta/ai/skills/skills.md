...
> **同步契约：** `_meta/system/scripts/sync-skills.py`
> **更新于：** 2026-06-10 — 采用两层结构（KOS + Utility），AH 已退役/冻结；W2 版本漂移修复完成；W3 退役标记部署完成
> **更新于：** 2026-09-20 — 归并收敛裁定：`.agents/skills/` 标记 deprecated（**非**本契约平台，不纳入同步）；`kos-novel` 从 `dee1d80^` 恢复入 canon（20 → 21 active）
...
| kos-research | KOS | kos-research/ | active | ✅ | — | 自主研究（搜索→抓取→综合） |
| kos-audit | KOS | kos-audit/ | active | ✅ | — | 审计管理与执行 |
| kos-code | KOS | kos-code/ | active | ✅ | — | MQL4 代码管理（工单→提交→状态追踪） |
| kos-think | Utility | kos-think/ | active | ✅ | — | 10 原则思维 |
| kos-life | Utility | kos-life/ | active | ✅ | — | 三支柱周期 |
...
| kos-novel | KOS | kos-novel/ | active | ✅ | templates/×5 | AI 长篇小说创作（四层架构 + 圣经契约 + 切片法） |

**合计：21 active**

> 2026-09-20 归并收敛裁定：`kos-novel` 于 2026-09-16 随 `.claude/skills/` 整目录删除而全库丢失
> （canon 从未持有），本轮从 `dee1d80^` 恢复入 canon 并纳入本契约，合计 20 → 21。
...
| 分类 | 数量 | 位置 |
|:----|:----:|:-----|
| Active | 21 | `_meta/ai/skills/canon/` |
| Retired | 9 | `.opencode/skills/`（原地保留） |
| Frozen | 6 | `.opencode/skills/`（原地保留） |
| **总计** | **36** | |

> **`.agents/skills/`（17 项 · 非 Active 计数）** —— 2026-06 遗留碎片，非本契约平台。
> 2026-09-20 裁定 deprecated，不再同步；其内容为 canon 的子集，权威版本以 canon 为准。
