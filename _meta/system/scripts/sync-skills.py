#!/usr/bin/env python3
"""
sync-skills.py — 技能体系收敛 · 同步契约（单文件权威定义）
============================================================================
将 _meta/ai/skills/canon/ 同步到各平台 skill 目录。

设计文档: _meta/design/ 内 2026-06-10「技能体系收敛」架构设计（内部，不发布）
决策记录: 同上 · 头脑风暴的「决策记录（2026-06-10）」节
============================================================================
...
三、注册表格式（_meta/ai/skills/skills.md）
---------------------------------------------------------------------------
技能注册表包含以下列用于同步决策:
...
| codex   | .codex/skills/                | 已部署   |
| flownote| .flownote/skills/             | 暂缓操作 |

注（2026-09-20 归并收敛裁定）：`.agents/skills/` **不是本契约的平台**，
即不纳入同步（技能体系收敛设计只认 opencode/flownote/codex/claude）。
该目录为 2026-06 遗留碎片，冻结于 2026-06-12，已标记 deprecated ——
见 `.agents/skills/DEPRECATED.md` 与 `_meta/ai/skills/registry.md`。

五、使用方法
---------------------------------------------------------------------------
  python _meta/system/scripts/sync-skills.py                        # 同步全部
  python _meta/system/scripts/sync-skills.py --target claude        # 仅 Claude
  python _meta/system/scripts/sync-skills.py --dryrun               # 预览变更
  python _meta/system/scripts/sync-skills.py --force                # 跳过确认覆盖
  python _meta/system/scripts/sync-skills.py --cleanup              # 清理旧备份
...
CANON_DIR = os.path.join(VAULT_ROOT, "_meta", "ai", "skills", "canon")
REGISTRY = os.path.join(VAULT_ROOT, "_meta", "ai", "skills", "skills.md")
LOG_FILE = os.path.join(VAULT_ROOT, "_meta", "system", "logs", "operations", "maintenance.md")
...
def sync_skill_to_platform(skill, platform, platform_dir, dryrun=False, force=False):
    ...
    if not os.path.isdir(target_skill):
        if dryrun:
            print(f"  [DRYRUN] → 复制: [{platform}] {skill_dir}/（全新技能，整个目录）")
            return {"action": "copy", "detail": "目标目录不存在（dryrun 跳过）"}
...
def main():
    parser = argparse.ArgumentParser(
        description="技能同步契约 — 将 _meta/ai/skills/canon/ 部署到各平台",
    )
    parser.add_argument(
        "--target",
    ...
    for platform in targets:
        ...
        if not os.path.isdir(platform_dir):
            # 2026-09-20 (D2 修复): 原为「WARNING + 跳过」，导致本表标注「需补齐」的
            # 平台永远无法被补齐 —— `.claude/skills/` 于 dee1d80(2026-09-16) 被删后，
            # 本脚本只会静默跳过，与其自身定位矛盾。现改为创建目录。
            if args.dryrun:
                print(f"\n[DRYRUN] 平台目录不存在，将创建: {platform_dir}")
            else:
                os.makedirs(platform_dir, exist_ok=True)
                print(f"\n[CREATE] 平台目录不存在，已创建: {platform_dir}")
