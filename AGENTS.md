...
> 本仓库是个人知识管理系统，**非软件项目** — 无构建系统、无测试套件、无应用代码。通过 Obsidian 管理，AI 辅助分拣、编译与链接校验。
...
├── _logs/            # 操作日志（operations/ + reports/）
├── _meta/            # 模板、ADR、查询、索引、ai-memory
├── _agents/          # AI 角色定义（kos/ + life/）
...
- `_logs/` 和 `0 Inbox/` 下的文件不要求完整 frontmatter
...
| **L2** | UDC 类号格式 + 是否在映射表中 | `_logs/`、`0 Inbox/`、`Periodic/`、`_meta/` |
| **L3** | `[[wiki链接]]` 和 `[md](link.md)` 目标存在性；循环引用 >10 跳报 info | 忽略外部 URL |
| **L4** | 三语言镜像齐全 + udc/tags 一致 | `_logs/`、`0 Inbox/`、`Periodic/` |
...
所有周期操作**幂等**（已有则跳过）。创建时使用 `_meta/Templates/` 对应模板。

| 操作 | 输出位置 | 关键输入 |
|------|---------|---------|
| `Daily Open` / `Day-Review` | `Periodic/YYYY/MM/YYYY-MM-DD.md` | 活跃工单未完成任务 |
| `Week-Review` | `_logs/reports/YYYY-WW.md` | triage/compile 统计 + 本周日记数 + 项目状态 |
| `Month-Review` | `_logs/reports/YYYY-MM.md` | 聚合本月周报 + 子库增长统计 |
| `Quarter-Review` | `_logs/reports/YYYY-QQ.md` | 聚合本月报 + 项目完成统计 |
| `Year-Review` | `_logs/reports/YYYY.md` | 聚合全季度 + ADR 变更历史 |
...
**结束：** 更新 `_meta/hot.md`（本会话摘要、关键进展、活跃线程、当前状态）→ **检查 `_meta/.locks/` 确认所有锁已释放（如有残留则 `python3 scripts/wiki-lock.py clear-stale --max-age 0` 强制清理并记录）** → 操作日志写入 `_logs/operations/maintenance.md` → 有未完成 Triage/Compile 时提醒用户。
...
- `CLAUDE.md` — 快速上手参考
- `_meta/🔗 知识关联/Index/_index-zh-cn.md` — 总索引
- `_meta/Templates/` — 全部模板
- `_meta/queries/` — Dataview 查询库（6 个预定义查询）
- `_meta/adr/` — 架构决策记录
- `_meta/ai-memory/全局状态.md` — 跨会话状态
- `_agents/life/` — Life+AI 三支柱角色定义
