...
新增笔记时按以下四层管道自动建议 UDC 类号，优先级从高到低：
...
| 目录模式 | 推断 UDC |
|---------|---------|
| `3 Resources/LLM-Wiki/*` | `004.8` |
| `3 Resources/PARA/*` | `001.8:005` |
| `3 Resources/UDC/*` | `025.4` |
| `3 Resources/People/*` | `929` |
| `_logs/operations/*` | `004.8:005.1` |
| `_logs/sessions/*` | `001.8:004.8` |
| `_logs/reports/*` | `001.8:311` |
| `Periodic/*` | `001.8:004.8` |
| `_meta/adr/*` | `001.8` |
| `_meta/design/*` | `001.8` |
| `_meta/queries/*` | `001.8:025.4` |
| `2 Areas/*` | `001.8` |

**Layer 3 — 关键词匹配**
扫描笔记标题和内容，对照 `_meta/udc-keywords.json` 中的映射表进行匹配。
映射表由现有 264 个已标注笔记自动生成，包含：
- `title_map`：标题名→UDC 映射（259 条）
- `keyword_map`：关键词→UDC 映射（52 条，置信度 >= 60%）
- `compound_rules`：正则规则匹配复合类号（10 条）

**Layer 4 — AI 内容分析（兜底）**
前三层无匹配时，由 Codex 读取笔记内容推断 UDC：

| 内容特征 | 推断 UDC |
|---------|---------|
| 含 transformer/attention/neural | `004.8:519.6` |
| 含 RAG/向量检索/Chroma/FAISS | `001.8:004.8` |
| 含 学习率/损失函数/训练 | `004.8:681.3` |
| 含 提示词/prompt/system/cognitive | `004.8:159.9` |
| 含 PARA/项目管理/GTD | `001.8:005` |
| 含 分类/索引/ontology/taxonomy | `025.4` |
| 含 人物/传记/生平 (非模板) | `929` |
| 无特征匹配 | `001.8`（通用兜底） |

**执行时机：**
- Codex 新建笔记时按四层管道填充 frontmatter 的 `udc` 字段
- KOS-Triage 分拣时对 target 文件建议 UDC
- KOS-Wiki-Compile 编译时对目标 Wiki 页面建议 UDC
- 建议值写入后标记 `udc:suggested` 以便人工审核

**更新映射表：**
当新增笔记达到一定数量时，重新运行 `1 Projects/UDC 自动建议/build_keyword_map.py` 以刷新映射表。
...
**维度 E — UDC 建议**
按以下优先级自动建议 UDC 类号：
1. 文件来源子库（路径匹配）→ 对照 `_meta/udc-keywords.json` 的 `directory_defaults`
2. 标题关键词匹配 → 对照 `title_map`（首选）和 `keyword_map`
3. 内容关键词匹配（正则）→ 对照 `compound_rules`
4. 以上均无匹配 → 标记 `udc: needs-mapping` 待人工确认
建议值写入 frontmatter 后标记 `udc.suggestion: auto`。
- < 500 字 + 单主题 → low
- 500–2000 字 + 多主题 → medium
- > 2000 字 或 含多个独立段 → high
...
2. 在目标文件顶部添加标准 frontmatter，包含 UDC 建议（按维度 E 规则）
...
**Step 3 — 页面创建/更新**
使用对应模板创建或更新 Wiki 页面：
- 目标页面不存在 → 按模板新建，自动建议 UDC（参考 `_meta/udc-keywords.json`）
- 目标页面已存在且 
eviewed: false → 合并更新，补充 UDC 若缺失
- 目标页面已存在且 
...
本分支在 KOS 基础上扩展 2 Areas/ 为生活/学习/工作三支柱结构，并引入 AI 层。

### AI 规则引用

详细规则见 `_agents/life/` 目录，按角色分文件：

| 文件 | 角色 | 职责 |
|------|------|------|
| `_agents/life/简报员.md` | 简报员 | 晨间提示 + 傍晚回顾 |
| `_agents/life/分析师.md` | 分析师 | 周/月趋势报告 |
| `_agents/life/协作者.md` | 协作者 | 目标进度异常提醒 |
...
用户写入 Period/ 日常笔记
  └─ 🏠 三支柱速写区块 (生活:: / 学习:: / 工作::)
       ↓
AI 每周汇总 → 2 Areas/{支柱}/日志/{YYYY-WW}.md
       ↓
AI 月汇总 → 2 Areas/{支柱}/日志/{YYYY-MM}.md
```
...
|---------|---------|---------|
| 简报员 | 会话开始 + 结束 | 提示用户后输出 |
| 分析师 | 周/月初始 | 提示可生成，确认后执行 |
| 协作者 | 每次会话 | 时间过半+完成<50%时提醒 |

### 内容归类原则

- 3 支柱按内容本身性质决定归属
- 生活领域使用 `#life/*` 标签，不使用 UDC
- 知识组织相关内容保留在 KOS 主分支
