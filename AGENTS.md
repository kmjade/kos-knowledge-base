...
**结束：** 更新 `_meta/hot.md`（本会话摘要、关键进展、活跃线程、当前状态）→ **检查 `_meta/.locks/` 确认所有锁已释放（如有残留则 `python3 scripts/wiki-lock.py clear-stale --max-age 0` 强制清理并记录）** → 操作日志写入 `_meta/system/logs/operations/maintenance.md` → 有未完成 Triage/Compile 时提醒用户。
