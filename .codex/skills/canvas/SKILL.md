---
name: canvas
description: "KOS 第三个知识捕获层 �?可视化参考层。将图片、文本卡片、PDF �?wiki 页面添加�?Obsidian Canvas 文件，支持区域内自动定位。触发词：canvas、画布、可视、visual、visualize、Canvas-Status、Canvas-New、Canvas-Add-Image、Canvas-Add-Text、Canvas-Add-Note、Canvas-Add-PDF、Canvas-Zone、Canvas-List"
...
三个知识捕获层：
- **KOS-Compile** �?文本综合（`wiki/` 页面�?- **KOS-Triage + KOS-Query** �?结构化知识（索引 + 查询�?- **Canvas** �?**可视化参�?*（`_attachments/canvases/` 画布�?
画布�?Obsidian 渲染为无限视觉面板的 JSON 文件（`.canvas`，JSON Canvas 1.0 格式）。本技能直接读写画�?JSON。先�?`json-canvas` 参考技能了解完整格式�?
---
...
### `Canvas-Status` �?状态查�?
1. 检�?`_attachments/canvases/_index.canvas` 是否存在
2. 如果存在：读取，按类型统计节点，列出所�?group 标签（区域名�?   - 报告�?画布�?N 个节点：X 个图片，Y 个文本卡片，Z 个笔记链接。区域：[列表]"
3. 如果不存在：用起始结构创�?4. 提示�?�?Obsidian 中打开 `_attachments/canvases/_index.canvas` 查看"

### `Canvas-New <name>` �?新建画布

1. 将名�?slugify：小写，空格→连字符，去特殊字符
2. 创建 `_attachments/canvases/[slug].canvas` 并用起始结构初始�?3. 报告：`"已创�?_attachments/canvases/[slug].canvas"`

### `Canvas-Add-Image <path>` �?添加图片

**解析图片�?*
- 如果�?URL（以 `http` 开头）：用 `curl -sL [url] -o _attachments/media/images/[filename]`
- 如果�?vault 外路径：复制�?`_attachments/media/images/`
- 如果已是 vault 内相对路径：直接使用

创建 `_attachments/media/images/` 如果不存在�?
**检测宽高比**（可选，仅当系统�?PIL/ImageMagick 时）�?```bash
python3 -c "from PIL import Image; img=Image.open('[path]'); print(img.width, img.height)"
```

| 宽高�?| 条件 | Canvas 宽度 | Canvas 高度 |
|--------|------|------------|------------|
...
**自动定位**（见下文算法）�?*追加节点到画�?JSON 并写入�?*

报告：`"已添�?[filename] �?[区域] 区域，位�?([x], [y])"`
...
1. �?vault 中搜索匹配的笔记文件（大小写不敏感，模糊匹配�?2. 使用 vault 相对路径作为 `file` 字段，`type: "file"`（不�?`"link"`�?3. 文件节点：width=300, height=100
4. 自动定位
...
与添加图片相同。Obsidian 原生渲染 PDF�?- vault 外路�?�?复制�?`_attachments/media/pdfs/`
- 固定尺寸：width=400, height=520

### `Canvas-Zone <name> [color]` �?添加区域分组

1. 读取画布 JSON
2. 找当前最�?y：`max(node.y + node.height for all nodes) + 60`
3. 创建 group 节点：x=-400, width=1000, height=400, color 默认�?"3"

有效颜色：`"1"`=�?`"2"`=�?`"3"`=�?`"4"`=�?`"5"`=�?`"6"`=�?
### `Canvas-List` �?列出所有画�?
1. `Glob _attachments/canvases/*.canvas`
2. 每个画布：读 JSON，统计节�?3. 报告�?```
...
def next_position(canvas_nodes, target_zone_label, new_w, new_h):
    ...
    if zone is None:
        # �?zone：放在所有内容下�?        max_y = max((n['y'] + n.get('height', 0) for n in canvas_nodes), default=-140)
        return -400, max_y + 60
...
- 一次操作建�?�? 个节�?- 不支持异步入�?- Canvas 内容不在 KOS-Query 范围内（JSON �?Markdown�?- 不自动裁�?压缩/转换图片格式
- 不跨语言翻译（视觉内容语言无关�?- 创建新画布时**不更新索�?*（画布是辅助可视层，非知识库主体�?
---

## 总结

1. 操作前置：先�?`json-canvas` 参考技能确认格�?2. 写入前始终读取画布文件，解析已有节点避免 ID 冲突
3. 外部资源复制�?`_attachments/media/` 对应子目�?4. 使用自动定位算法维持布局整洁
5. 写入操作需文件锁保�?6. 报告每次操作的位置和区域

## 参见

- `json-canvas` 参考技能：JSON Canvas 1.0 格式规范
- `_meta/design/canvas-可视层设�?md`：完整设计文�?